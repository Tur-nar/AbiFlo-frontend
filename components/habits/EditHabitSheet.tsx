...
import { useUpdateHabit } from "@/hooks/use-habit";
import { useCategories } from "@/hooks/use-category";
import { CategoryIcon } from "@/components/common/CategoryIcon";
import { toast } from "sonner";
import { Loader2 } from "lucide-react";
import type { Habit, UpdateHabitPayload, Frequency, Difficulty } from "@/types/habit.types";
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="w-full sm:max-w-[420px] overflow-y-auto border-l border-border/50 bg-card p-0">
          <SheetHeader className="px-5 pt-5 pb-3 border-b border-border/30">
            <SheetTitle className="text-base font-semibold">Edit Habit</SheetTitle>
          </SheetHeader>
          <ScrollArea className="h-[calc(100vh-64px)]">
            <div className="px-5 py-4">
              <EditHabitForm habit={habit} onOpenChange={onOpenChange} />
            </div>
          </ScrollArea>
        </SheetContent>
      </Sheet>
    ...
    <Drawer open={open} onOpenChange={onOpenChange} showSwipeHandle>
      <DrawerContent className="max-h-[90dvh]">
        <DrawerHeader className="px-5 pt-4 pb-2">
          <DrawerTitle className="text-base font-semibold">Edit Habit</DrawerTitle>
        </DrawerHeader>
...
function EditHabitForm({
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      habitType: habit.habitType as "BUILD" | "QUIT",
      frequency: habit.frequency as "DAILY" | "WEEKLY" | "CUSTOM",
      frequencyDays: habit.frequencyDays ?? [1, 2, 3, 4, 5],
      difficulty: habit.difficulty as "EASY" | "MEDIUM" | "HARD" | "EXTREME",
      isMeasurable: !!(habit.unit || habit.targetValue),
      unit: habit.unit ?? "",
        ...
        {isMeasurable && (
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              ...
              <Input placeholder="min, km..." {...form.register("unit")} className="bg-background/50 text-sm h-8" />
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] uppercase text-muted-foreground">Target</Label>
              <Input type="number" placeholder="60" {...form.register("targetValue")} className="bg-background/50 text-sm h-8" />
            </div>
          </div>
        )}
      </div>
